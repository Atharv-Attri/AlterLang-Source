...
def main():
    ...
    for i in filed:
        if i["live"] == "0":
            total += 1
            out = subprocess.Popen(f"python -m compiler tests/{i['file']}", stdout=subprocess.PIPE)
            ...
            error = False
            rich.print(
                Panel(f"[bold blue]Running {i['file']}...[/bold blue]"))
            ...
            for j in outputs:
                ...
                a+=1
            if not error:
                rich.print(f"[bold green]Tests for {i['file']} were a success![/bold green]")
                passed += 1
            if error:
